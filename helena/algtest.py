...
# On/Off correlation
    if(True): #statusKey):
     if(buffLen>=elementsNumberOnBed and counterTime%timeCheckingOnBed == 0 ):
       ...
       saveResults(unit, 'corrStatus', 'bs10' ,str(peaksCR), nowtime)
       if(peaksCR > thccMean):
           prevalues.append(1)
       else:
           prevalues.append(0)
       ...
       if(counteron > thon):
          ...
          if(enablesmson and onBed == False):
             ...
             sendEmailT.start()
          saveResults(unit, 'bedStatus', 'bs' ,'1', buffertime[len(buffertime)-thon])
          onBed = True
          pOnBed = True
          ...
          if(enablesmsoff and onBed == True):
             ...
             sendEmailT.start()
          #saveOFF
          saveResults(unit, 'bedStatus', 'bs' ,'0', buffertime[len(buffertime)-thoff])
          onBed  = False
          pOnBed = False
          hrSignalNoNoise = 0
       else:
          #Calculing
          saveResults(unit, 'bedStatus', 'bs' ,'2', buffertime[len(buffertime)-1])
          pOnBed = True
...
#            if(debug): print "HBR greater than 30 --> ",hbr
    ...
    #################################################################
     ...
     if(counterStable == postureChangeTimeWindow + 5):
       ...
       nowtime = buffertime[len(buffertime)-1]
       saveResults(unit, 'change', 'x' ,str(percent), nowtime)
     # previousHBSignal
