...
# declare -a iplist=("10.0.0.84" "10.0.0.4" "10.0.0.223" "10.0.0.225")
declare -a iplist=("10.0.0.213" "10.0.0.227")

for ip in "${iplist[@]}"
do
   echo "set up: $ip"
   scp -i ~/bdot.pem build/beddot.tar.gz  myshake@$ip:/opt/ && ssh -i ~/bdot.pem -t myshake@$ip 'sudo systemctl stop helena && sudo systemctl stop influxshake && cd /opt/ && tar -xvf beddot.tar.gz && sudo systemctl start influxshake && sudo systemctl start helena'
